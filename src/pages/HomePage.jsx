    ...
    <div className="min-h-screen flex flex-col text-slate-800">
      ...
      <main className="flex-grow max-w-5xl w-full mx-auto p-4 md:p-8">
        <div className="text-center mb-10 max-w-2xl mx-auto">
          ...
          <p className="text-slate-600 mt-2">
            Select an exercise type below to start improving your English skills.
          </p>
        </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <h3 className="font-extrabold text-slate-900 text-lg title-font mb-2">{type.title}</h3>
                  <p className="text-slate-500 text-xs leading-relaxed mb-2">{type.description}</p>
                  <div className="flex items-center space-x-1.5 mt-3 mb-6">
                    <span className={`w-2 h-2 rounded-full ${colors.dot}`}></span>
                    <span className="text-xs text-slate-500 font-medium">{type.count} exercises available</span>
                  </div>
                </div>
                <button
      ...
      <footer className="bg-white border-t border-slate-200 py-4 px-6 text-center text-xs text-slate-400">
        <div className="max-w-5xl mx-auto">
          <span>© 2026 Easy English Learning Hub. Built for A2+ Learners.</span>
        </div>
      </footer>
    </div>
  );
